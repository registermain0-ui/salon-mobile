...
// SNS用: 通常24h表記(0〜9時は0埋めしない。M-V13.3: 「8:20」「0:15」のように直接時間で表示)
...
function fmtMsgTime(min) { // ★M-V13.3: 出力統一のため24時以降は巻き戻す表記(最短取得コピーと同じfmtNormal仕様)
...
function reportFormatLine(r) {
  const start = fmtNormal(r.start); // ★M-V13.3: 出力統一のため24時以降は巻き戻す表記
...
const BACK_CATEGORY_LABEL = { free: "フリー", hime: "姫予約", hon: "本指名" };
...
// ★M-V13.3: 予約入力時、OPが付く場合(9割以上=MB)はopFlagを空欄のまま運用し、
// OPが付かない時だけ「無」を明示選択する運用のため、空欄は「MB」を選んだものとして扱う。
// 「無」が明示されている時だけ0円とする。)
function resolveOpPrice(opFlag, prices, cfgOptions) {
  const flag = opFlag || "";
  if (flag === "無") return 0;
  const opDef = (cfgOptions || []).find(o => o.name === flag);
  if (opDef) return Math.max(0, opDef.price || 0);
  if (flag === "有") return Math.max(0, (prices && prices.opPrice) || 0);
  // 空欄(未選択) → MBが付いているものとして扱う
  const mb = (cfgOptions || []).find(o => o.name === "MB");
  if (mb) return Math.max(0, mb.price || 0);
  return Math.max(0, (prices && prices.opPrice) || 0); // MB未設定時のフォールバック(旧互換の単一OP単価)
}

/* 予約1件の「システム計算上の預かり金期待値」(コース+延長+OP+指名料-割引)。
 * ★M-V13.3: 精算では resolveOpPrice の空欄=MB前提で再計算するため、
...
function calcExpectedDeposit(r, prices, cfgOptions, cfgNomTypes, therapist) {
  ...
  const extAmount = units * Math.max(0, prices.extensionUnitPrice || 0);
  const opAmount = resolveOpPrice(r.opFlag, prices, cfgOptions);
  const nomDef = (cfgNomTypes || []).find(n => n.name === r.nominationType);
  const applyFee = nomDef ? !!nomDef.fee : r.nominationType === "本";
  const nomFeeAmount = applyFee ? Math.max(0, (therapist && therapist.nominationFee) || 0) : 0;
  const discount = Math.max(0, r.discountAmount || 0);
  return courseAmount + extAmount + opAmount + nomFeeAmount - discount;
}

/* 1日分(1セラピスト)の精算サマリを計算
 * reservations: そのセラピスト・その日の予約配列
...
function buildSeisanSummary(reservations, prices, rates, cfgOptions, cfgNomTypes, therapist, entry, roundBaseWoman, roundBaseShop) {
  ...
  for (const r of reservations) {
    ...
    const expected = calcExpectedDeposit(r, prices, cfgOptions, cfgNomTypes, therapist); // ★M-V13.3: OPの空欄=MB前提で再計算
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    backCategoryOf, roundToThousand, resolveOpPrice, calcExpectedDeposit, buildSeisanSummary, checkBackRateAlert, buildSeisanMessage
  };
}
...
const K = {
  ...
  seisan: d => `este.seisan.${d}`,        // ★M-V13.3: 精算データ(当日限り)
  seisanFormat: "este.seisanFormat",      // ★M-V13.3: 精算送信文フォーマット
...
function loadSeisan(d) { return LS.get(K.seisan(d), []); } // ★M-V13.3
function saveSeisan(d, list) { LS.set(K.seisan(d), list); touchMeta(K.seisan(d)); } // ★M-V13.3
const DEFAULT_SEISAN_FORMAT = { // ★M-V13.3
  header: "本日の、報酬をお送りいたします！\n後ほど、ご確認を宜しくお願い致します🙆‍♂️",
  footer: "この後、ルーム使用をしますので\n電気、エアコンはつけたままで大丈夫です🙇‍♂️\n補充、清掃、洗濯などは\nお願い致します🙆‍♂️"
};
function loadSeisanFormat() { // ★M-V13.3
...
function saveSeisanFormat(f) { LS.set(K.seisanFormat, f); touchMeta(K.seisanFormat); } // ★M-V13.3
...
const DEFAULT_SETTINGS = {
  ...
  seisan: { roundBaseWoman: 500, roundBaseShop: 750, alertMode: "fixed", alertFixedPercent: 50 } // ★M-V13.3
};
...
function reloadDate() {
  ...
  state.seisan = loadSeisan(state.dateKey); // ★M-V13.3
  state.holdCells = {};
...
function attAddRow(t, cur, focusStart) {
  ...
  const wrap = document.createElement("div"); // ★M-V13.3: 出勤行+SNS表示補足をまとめる当日限りの入れ物
  ...
  const sns = document.createElement("input"); // ★M-V13.3: SNS表示補足(この日の出勤データにのみ紐づく・翌日には残らない)
...
document.getElementById("attSave").addEventListener("click", () => {
  ...
  for (const wrap of document.querySelectorAll("#attRows .att-entry")) {
    ...
    const snsSuffix = wrap.querySelector(".att-sns").value.trim(); // ★M-V13.3
...
function renderShortest() {
  ...
  const snsSuffixOf = new Map(state.attendance.map(a => [a.therapistId, (a.snsSuffix || "").trim()])); // ★M-V13.3: この日の出勤データから(翌日には残らない)
  ...
  for (const c of cands) {
    ...
    if (c.maxMinutes > 0) {
      ...
      // ★M-V13.3: 最短取得の画面からSNS表示補足を直接編集(出勤登録を開かずに済む・当日のみ有効)
  ...
  for (const c of cands) {
    ...
    const suffix = snsSuffixOf.get(c.therapistId) || ""; // ★M-V13.3: セラピスト別「〜」の後の固定文言(当日のみ)
...
// ★M-V13.3.1: 検索入力→サジェストではなく、タップ→プルダウン選択の2タップで追加できるようにする
function renderSeisanAddSelect() {
  const sel = document.getElementById("seisanAddSelect");
  const added = seisanAddedIds();
  const list = presentTherapists().map(p => p.t)
    .filter(t => !added.has(t.id))
    .sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase(), "ja"));
  sel.innerHTML = `<option value="">＋ セラピストを追加</option>` +
    list.map(t => `<option value="${t.id}">${esc(t.name)}</option>`).join("");
  sel.disabled = list.length === 0;
}
document.getElementById("seisanAddSelect").addEventListener("change", (e) => {
  const id = Number(e.target.value);
  if (!id) return;
  state.seisan.push({
    therapistId: id, startingCash: null, transportFee: null,
...
function buildSeisanCard(entry, idx) {
  ...
  del.textContent = "×";
  del.setAttribute("aria-label", "このセラピストの精算を削除");
  del.addEventListener("click", () => {
  ...
  // ★M-V13.3.1: バック率・交通費はタップ→選択のプルダウンに変更(要望対応)
  const selectInput = (className, options, val) => {
    const s = document.createElement("select");
    s.className = className;
    const curStr = (val != null && val !== "") ? String(val) : "";
    let opts = options;
    if (curStr !== "" && !options.some(o => o.value === curStr)) {
      opts = [{ value: curStr, label: `${curStr}(現在の値)` }, ...options];
    }
    for (const o of opts) {
  ...
  const rateOptions = [
    { value: "", label: "未使用" },
    { value: "50", label: "50%" }, { value: "55", label: "55%" }, { value: "60", label: "60%" },
    { value: "65", label: "65%" }, { value: "70", label: "70%" }, { value: "75", label: "75%" }
  ];
  const transportOptions = [{ value: "0", label: "無" }, { value: "1000", label: "1000円" }];
  ...
  // 開始金(単独行・重要項目なので大きめ表示)
  const rowStart = document.createElement("div"); rowStart.className = "sz-row";
  const inStart = numInput(entry.startingCash, "0");
  inStart.classList.add("sz-num-lg");
  rowStart.append(fg("開始金(円)", inStart));
  card.appendChild(rowStart);

  // 雑費 名称
  const rowMiscName = document.createElement("div"); rowMiscName.className = "sz-row";
  const inMiscName = document.createElement("input");
  inMiscName.className = "sz-text";
  inMiscName.placeholder = "例: 備品代(任意)";
  if (entry.miscName) inMiscName.value = entry.miscName;
  rowMiscName.append(fg("雑費 名称(任意)", inMiscName));
  card.appendChild(rowMiscName);

  // 交通費(選択式)・雑費 金額
  const row2 = document.createElement("div"); row2.className = "sz-row";
  const inTransport = selectInput("sz-select", transportOptions, entry.transportFee);
  const inMiscAmount = numInput(entry.miscAmount, "0");
  row2.append(fg("交通費", inTransport), fg("雑費 金額(円)", inMiscAmount));
  card.appendChild(row2);

  // バック率(選択式・3列シンプル表示)
  const row3 = document.createElement("div"); row3.className = "sz-row sz-row-3";
  const inRateFree = selectInput("sz-select", rateOptions, entry.rateFree);
  const inRateHime = selectInput("sz-select", rateOptions, entry.rateHime);
  const inRateHon = selectInput("sz-select", rateOptions, entry.rateHon);
  row3.append(fg("フリー(%)", inRateFree), fg("姫予約(%)", inRateHime), fg("本指名(%)", inRateHon));
  card.appendChild(row3);

  // 予約一覧+預かり金入力
  const reservations = seisanReservationsFor(entry.therapistId);
  const resSection = document.createElement("div");
  resSection.className = "sz-section";
  const resTitle = document.createElement("div");
  resTitle.className = "sz-section-title";
  resTitle.textContent = "本日の予約・預かり金";
  ...
  if (reservations.length === 0) {
    ...
    for (const r of reservations) {
      ...
      resSection.appendChild(expectLine);
    }
  }
...
function updateSeisanResult(entry, card) {
  ...
  html += `<span class="sz-final">釣り銭(開始金+店落ち) → ${summary.change.toLocaleString()}円</span>`;
  if (alert2) {
    html += `<div class="sz-alert">⚠ 女性給が想定より低くなっています。バック率の入力を確認してください。</div>`;
  }
  result.innerHTML = html;
}
const BACK_CATEGORY_LABEL_JS = { free: "フリー", hime: "姫予約", hon: "本指名" };
...
document.getElementById("szMsgReset").addEventListener("click", () => { // ★M-V13.3
...
document.getElementById("setSave").addEventListener("click", () => {
  ...
  saveSeisanFormat({ header: seisanHeader, footer: seisanFooter }); // ★M-V13.3
  CFG = loadSettings();
...
document.getElementById("openTodayMenu").addEventListener("click", () => { closeSheets(); openSheet(document.getElementById("todayMenuSheet")); }); // ★M-V13.3
document.getElementById("openOtherMenu").addEventListener("click", () => { closeSheets(); openSheet(document.getElementById("otherMenuSheet")); }); // ★M-V13.3
